public class Facturation{
        ...
        public void addFacture(Facture facture){
            this.factures[lastIndexFacture++] = facture;
        }
        ...
        public double quantiteVendue(String descP){
            double quantite=0;
            for(int i=0;i<lastIndexFacture;i++){
                for(int j=0;j<factures[i].getLigneFactures().length;j++){
                    if(descP.equalsIgnoreCase(factures[i].getLigneFactures()[j].getProduitAcquis().getDescProduit())){
